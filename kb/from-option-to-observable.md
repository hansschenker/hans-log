...
description: Option monad → Observable, a monadic journey (NotebookLM)
resource: D:\Learning-Local-Hanss\Rxjs-From-Option-to-Observable-Piechoki
...
- **This is the missing bridge in the RxJS-from-FP story.** [rxjs-from-fp-js-to-rxjs](./rxjs-from-fp-js-to-rxjs.md) promises
...
- **Teaching order matters.** The talk earns `flatMap` by first showing `map` producing
  ...
  [RxJS Operator Renaming — The Suffix Grammar](./rxjs-operator-renaming.md): `flatMap` → `bind` → `chain` → `concatMap`/`mergeMap` is exactly the
...
- Piechocki's own framing — intuition first, category theory never — is the right stance for the
  course material; contrast with [erik-meijer](./erik-meijer.md) duality, which is the deep-theory end of the same
  spectrum.

## Related

- [rxjs-from-fp-js-to-rxjs](./rxjs-from-fp-js-to-rxjs.md) — the FP-concepts-on-streams course this talk motivates
- [rxjs-fp — A Functional-Style RxJS Built From Scratch](./rxjs-fp.md) — from-scratch functional RxJS (curried free operators, cold core)
- [Pipe vs Compose — Point-Free Composition in RxJS FP Architecture](./rxjs-pipe-compose.md) — point-free composition; the pipeline-of-transformations idea applied to RxJS
- [RxJS Operator Renaming — The Suffix Grammar](./rxjs-operator-renaming.md) — `flatMap`/`bind`/`chain`/`mergeMap` as one root under many names
- [erik-meijer](./erik-meijer.md) — Iterable/Observable duality, the theory end of the same story
- [js-functional-programming-nlm](./js-functional-programming-nlm.md) — the 12-module FP-in-JavaScript course
- [Professor Frisby's Mostly Adequate Guide to Functional Programming](./fp-guide.md) — Mostly Adequate Guide, the book-length treatment of the same path to monads
- [universal-algebra](./universal-algebra.md) — operations-and-laws framing behind the monadic laws
