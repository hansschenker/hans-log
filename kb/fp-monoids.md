...
description: multiplicative monoid, concat/empty laws in the crocks library
resource: https://crocks.dev/docs/monoids/Prod.html
...
- **`Endo` is the interesting one** — function composition as a monoid, identity = the identity
  ...
  [Professor Frisby's Mostly Adequate Guide to Functional Programming](./fp-guide.md) Ch. 5.
- **`First`/`Last` over `Maybe`** turn "take the first present value" into a fold — the monoid form
  ...
  [From Options to Observables — a monadic journey (Miłosz Piechocki, WarsawJS #43)](./from-option-to-observable.md).
...
- crocks is the FP ADT library the other 2026-08-13 crocks entries point at:
  [crocks Combinators — applyTo, composeB, converge, psi, substitution & friends](./fp-combinators.md), [crocks Functions — the whole library index, by category](./fp-functions.md)

## Notes

- **This is the API view of [Professor Frisby's Mostly Adequate Guide to Functional Programming](./fp-guide.md) Ch. 13.** The book puts "Monoids bring it all together"
...
- **`Endo` is the link to composition.** Function composition being a monoid is why
  [Pipe vs Compose — Point-Free Composition in RxJS FP Architecture](./rxjs-pipe-compose.md) can claim `pipe`/`compose` differ only in direction and intent — the
...
- **Open thread for RxJS:** `merge`, `concat` and `reduce` on Observables feel monoid-shaped
  ...
  [rxjs-from-fp-js-to-rxjs](./rxjs-from-fp-js-to-rxjs.md) easier to teach — this is the same "one root, many names" problem as
  [RxJS Operator Renaming — The Suffix Grammar](./rxjs-operator-renaming.md).
...
- [Professor Frisby's Mostly Adequate Guide to Functional Programming](./fp-guide.md) — Ch. 13 "Monoids bring it all together", the narrative version
- [crocks Combinators — applyTo, composeB, converge, psi, substitution & friends](./fp-combinators.md) — crocks combinator helpers
- [crocks Functions — the whole library index, by category](./fp-functions.md) — crocks point-free function index
- [universal-algebra](./universal-algebra.md) — algebra as operations plus laws, the frame monoids sit in
- [From Options to Observables — a monadic journey (Miłosz Piechocki, WarsawJS #43)](./from-option-to-observable.md) — `Maybe`/`Option` semantics behind `First` and `Last`
- [Pipe vs Compose — Point-Free Composition in RxJS FP Architecture](./rxjs-pipe-compose.md) — composition and associativity in the RxJS layer
- [js-functional-programming-nlm](./js-functional-programming-nlm.md) — the 12-module FP-in-JavaScript course
