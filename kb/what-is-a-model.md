...
description: model as static blueprint vs dynamic state machine — from FSM theory to distributed retry architecture
resource: ./what-is-a-model.pdf
...
Source deck generated with NotebookLM: **The Architect's Blueprint to the Living System — Unifying Abstract Computer Science Models with Distributed Production Systems** (14 slides). See `./what-is-a-model.pdf`.
...
`./what-is-a-model.pdf` (NotebookLM slide deck, "Model as Abstraction")
...
- [staltz-two-fundamental-abstractions](./staltz-two-fundamental-abstractions.md) — Staltz's reactive abstractions, the continuous/streaming side of the same coin
- [Effects as Data | Richard Feldman | Reactive 2015](./effects-as-data-richard-feldman-reactive-2015.md) — the same Model → State → Stream idea from the effects angle; Elm's `update : Action -> Model -> (Model, Effects)` is this note's state machine made concrete
- [ReactiveConf 2016 - André Staltz: Visualizing the data flow with Cycle.js](./reactiveconf-2016-andr-staltz-visualizing-the-data-flow.md) — Cycle.js as the continuous/streaming corner of the triad; data flow made visible via the stage-observes-world cycle
