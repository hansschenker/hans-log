...
description: the classic FP-in-JavaScript book, currying → monads
resource: https://github.com/MostlyAdequate/mostly-adequate-guide
...
- **"Coding by composing"** — programs as `f∘g∘h` pipelines rather than statement sequences, the same
  idea [Pipe vs Compose — Point-Free Composition in RxJS FP Architecture](./rxjs-pipe-compose.md) examines from the RxJS side.
...
- **"Monadic Onions"** (Ch. 9) — nesting is the problem, `join`/`chain` is the answer. Exactly the
  `Option<Option<T>>` motivation used in [From Options to Observables — a monadic journey (Miłosz Piechocki, WarsawJS #43)](./from-option-to-observable.md), reached from the container
...
- **Monoids "bring it all together" (Ch. 13)** — the closing chapter, not the opening one: an
  ...
  structure the crocks `Prod` docs in [crocks Monoids — Prod, and the shared empty/concat interface](./fp-monoids.md) present as an API.
...
same instinct behind [From Options to Observables — a monadic journey (Miłosz Piechocki, WarsawJS #43)](./from-option-to-observable.md), where the speaker explicitly refuses the
...
- **This is the spine the other 2026-08-13 entries hang off.** The crocks docs
  ([crocks Combinators — applyTo, composeB, converge, psi, substitution & friends](./fp-combinators.md), [crocks Monoids — Prod, and the shared empty/concat interface](./fp-monoids.md), [crocks Functions — the whole library index, by category](./fp-functions.md)) are the *reference* for the same concepts;
  this is the *narrative*. Read the guide for order and motivation, hit crocks when you need the API.
- **Ch. 8–9 is the pairing to make with the monad talk.** [From Options to Observables — a monadic journey (Miłosz Piechocki, WarsawJS #43)](./from-option-to-observable.md) arrives at
...
- **Open question for the course:** the guide never reaches streams. The natural extension is a
  ...
  [rxjs-from-fp-js-to-rxjs](./rxjs-from-fp-js-to-rxjs.md) is meant to fill. Worth checking whether the guide's chapter order
...
- [From Options to Observables — a monadic journey (Miłosz Piechocki, WarsawJS #43)](./from-option-to-observable.md) — the same path to monads, told in 30 minutes from the null-check side
- [crocks Monoids — Prod, and the shared empty/concat interface](./fp-monoids.md) — crocks `Prod`, the API view of Ch. 13
- [crocks Combinators — applyTo, composeB, converge, psi, substitution & friends](./fp-combinators.md) — crocks combinator helpers, the API view of Ch. 4–5
- [crocks Functions — the whole library index, by category](./fp-functions.md) — crocks point-free function index, the API view of Appendix C
- [JavaScript — Functional Programming for JavaScript Developers (Packt code repo)](./js-fp.md) — Packt FP-for-JS-developers course repo, the video-course counterpart
- [js-functional-programming-nlm](./js-functional-programming-nlm.md) — the 12-module NotebookLM FP course
- [rxjs-from-fp-js-to-rxjs](./rxjs-from-fp-js-to-rxjs.md) — the same 12 concepts rebuilt on streams
- [Pipe vs Compose — Point-Free Composition in RxJS FP Architecture](./rxjs-pipe-compose.md) — composition in the RxJS layer
- [universal-algebra](./universal-algebra.md) — operations-and-laws framing behind functors, monads, monoids
