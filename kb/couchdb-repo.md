...
description: offline-first sample app repo
resource: https://github.com/deroux/offline-first-apps-angular-ionic-couchdb
...
- **Changes feed → Subject bridge** — `db.changes(...).on('change', d => subject.next(d.doc))` turns
  DB mutations into an Observable stream (Promise/event → Observable, same move as [RxJS & PouchDB — Persistent Data Flows](./pouchdb-article.md)).
...
Promise→Observable adaptation Atencio describes in [RxJS & PouchDB — Persistent Data Flows](./pouchdb-article.md).
...
- **The offline-first magic is almost entirely `db.sync(remote, {live, retry})`** — PouchDB's
  ...
  `bufferWhen` write-batching Atencio hand-rolls in [RxJS & PouchDB — Persistent Data Flows](./pouchdb-article.md) — this repo delegates all of
  that to `sync`.
...
- [RxJS & PouchDB — Persistent Data Flows](./pouchdb-article.md) — RxJS + PouchDB persistent data flows (the operator-level view of the same idea)
- [rxjs-course-modules-drive](./rxjs-course-modules-drive.md) — RxJS Deep Dive course modules (same-day study batch)
