...
description: "André Staltz: Visualizing the data flow with Cycle.js — visualize reactive dataflow in Cycle.js"
resource: https://www.youtube.com/watch?v=3a98OPJWFPY
tags: [rxjs, frp, cyclejs]
...
- [What Is a Model](./what-is-a-model.md) — Cycle.js is the *continuous / streaming* corner of that note's Model → State → Stream triad; the stage-observing-world cycle is data flow made visible.
- [Effects as Data | Richard Feldman | Reactive 2015](./effects-as-data-richard-feldman-reactive-2015.md) — same ReactiveConf lineage and the same move: keep business logic pure (the stage / stateless functions) and push effects to the edge (drivers / `runAction`). Cycle's "drivers observe the stage" is Feldman's "represent effects as data" in another shape.
