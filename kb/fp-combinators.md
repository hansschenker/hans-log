...
description: combinator helpers (composeB, substitution, applyTo) in the crocks FP library
resource: https://crocks.dev/docs/functions/combinators.html
...
- **`identity :: a -> a`** — returns its input unchanged. The docs call it "a workhorse": it's the
  ...
  [crocks Monoids — Prod, and the shared empty/concat interface](./fp-monoids.md).
...
- Sibling crocks entries: [crocks Monoids — Prod, and the shared empty/concat interface](./fp-monoids.md), [crocks Functions — the whole library index, by category](./fp-functions.md)
...
- **This is the API view of [Professor Frisby's Mostly Adequate Guide to Functional Programming](./fp-guide.md) Ch. 4–5.** Currying and "coding by composing" are where the
...
- **Open thread for RxJS:** `converge` and `psi` are the static-time analogues of what
  ...
  [rxjs-from-fp-js-to-rxjs](./rxjs-from-fp-js-to-rxjs.md): the same shape, once over values and once over time.
...
- [Professor Frisby's Mostly Adequate Guide to Functional Programming](./fp-guide.md) — Ch. 4 Currying, Ch. 5 Coding by Composing, Appendix C Point-free Utilities
- [crocks Monoids — Prod, and the shared empty/concat interface](./fp-monoids.md) — `Endo`, where `identity` is the empty element
- [crocks Functions — the whole library index, by category](./fp-functions.md) — the wider crocks point-free function index
- [Pipe vs Compose — Point-Free Composition in RxJS FP Architecture](./rxjs-pipe-compose.md) — `pipe` vs `compose`, the same composition question in RxJS
- [universal-algebra](./universal-algebra.md) — operations-and-laws framing
- [From Options to Observables — a monadic journey (Miłosz Piechocki, WarsawJS #43)](./from-option-to-observable.md) — the ADT side of the same library ecosystem
